import { useState } from 'react';
import { useNavigate, Link } from 'react-router-dom';
import api from '../services/api';

export default function Register() {
  const navigate = useNavigate();
  const [form, setForm] = useState({ name: '', email: '', whatsapp: '', password: '', confirm: '', age: '' });
  const [error, setError] = useState('');

  function handleChange(e) {
    setForm(prev => ({ ...prev, [e.target.name]: e.target.value }));
  }

  async function handleSubmit(e) {
    e.preventDefault();
    setError('');
    if (form.password !== form.confirm) {
      setError('As senhas não coincidem.');
      return;
    }
    const age = parseInt(form.age);
    if (!form.age || age < 18) {
      setError('Você precisa ter pelo menos 18 anos.');
      return;
    }
    try {
      await api.post('/users/register', {
        name: form.name,
        email: form.email,
        whatsapp: '55' + form.whatsapp.replace(/\D/g, ''),
        password: form.password,
        age,
      });
      navigate('/login');
    } catch {
      setError('Erro ao criar conta. Verifique os dados.');
    }
  }

  return (
    <div style={styles.container}>
      {/* Fundo Orgânico/Futurista Padronizado */}
      <svg style={styles.bgSvg} viewBox="0 0 1440 900" preserveAspectRatio="xMidYMid slice">
        <path d="M-100,200 C100,250 150,450 50,600 C-50,750 -200,700 -250,550 Z" fill="url(#leafGrad)" opacity="0.4" filter="blur(40px)" />
        <path d="M1500,100 C1350,150 1200,300 1300,500 C1400,700 1550,650 1600,500 Z" fill="url(#leafGrad)" opacity="0.35" filter="blur(50px)" />
        <g stroke="#94D2BD" strokeWidth="1" opacity="0.5" fill="none">
          <circle cx="200" cy="150" r="3" fill="#94D2BD" />
          <circle cx="280" cy="110" r="4" fill="#94D2BD" />
          <line x1="200" y1="150" x2="280" y2="110" />
          <circle cx="1200" cy="400" r="3" fill="#94D2BD" />
          <circle cx="1280" cy="350" r="3" fill="#94D2BD" />
          <line x1="1200" y1="400" x2="1280" y2="350" />
        </g>
        <defs>
          <linearGradient id="leafGrad" x1="0%" y1="0%" x2="100%" y2="100%">
            <stop offset="0%" stopColor="#40916C" />
            <stop offset="100%" stopColor="#A9D6E5" />
          </linearGradient>
        </defs>
      </svg>

      <div style={styles.contentWrapper}>
        <div style={styles.card}>
          <div style={styles.logo}>
            <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#2D6A4F" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
              <path d="M11 20A7 7 0 0 1 9.8 6.1C15.5 5 17 4.48 19 2c0 2-.52 3.5-1.6 9.2A7 7 0 0 1 11 20z" />
              <path d="M19 2c-2.26 4.33-5.27 7.14-8 10" />
            </svg>
            <div>
              <div style={styles.logoTitle}>TagReativa</div>
              <div style={styles.logoSub}>Criar credencial de tutor</div>
            </div>
          </div>

          <form onSubmit={handleSubmit} style={styles.form}>
            <input style={styles.input} name="name" placeholder="Nome Completo" value={form.name} onChange={handleChange} required />
            <input style={styles.input} name="email" type="email" placeholder="E-mail de contato" value={form.email} onChange={handleChange} required />

            <div style={styles.whatsappRow}>
              <span style={styles.whatsappPrefix}>+55</span>
              <input
                style={styles.whatsappInput}
                name="whatsapp"
                placeholder="DDD + Número (ex: 83912345678)"
                value={form.whatsapp}
                onChange={handleChange}
                required
              />
            </div>

            <input style={styles.input} name="age" type="number" placeholder="Idade (Mínimo 18 anos)" value={form.age} onChange={handleChange} min={18} required />
            <input style={styles.input} name="password" type="password" placeholder="Definir Senha" value={form.password} onChange={handleChange} required />
            <input style={styles.input} name="confirm" type="password" placeholder="Confirmar Senha" value={form.confirm} onChange={handleChange} required />

            <div style={styles.notice}>
              O WhatsApp é o canal exclusivo de telemetria e resgate. Os dados de contato permanecerão ocultos e só serão disparados caso o dispositivo mude para o status "Perdido".
            </div>

            {error && <p style={styles.error}>{error}</p>}
            <button style={styles.button} type="submit">Registrar no Ecossistema</button>
          </form>

          <Link to="/login" style={styles.link}>Já possui conta vinculada? Acessar</Link>
        </div>
      </div>
    </div>
  );
}

const styles = {
  container: { minHeight: '100vh', position: 'relative', overflowX: 'hidden', background: 'linear-gradient(135deg, #F0F4F2 0%, #E2ECE9 50%, #D4E5E0 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: '-apple-system, BlinkMacSystemFont, sans-serif' },
  bgSvg: { position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', pointerEvents: 'none', zIndex: 1 },
  contentWrapper: { position: 'relative', zIndex: 2, padding: '32px 16px', width: '100%', display: 'flex', justifyContent: 'center' },
  card: { background: 'rgba(255, 255, 255, 0.85)', backdropFilter: 'blur(16px)', WebkitBackdropFilter: 'blur(16px)', borderRadius: '24px', padding: '36px 28px', width: '100%', maxWidth: '420px', boxSizing: 'border-box', boxShadow: '0 12px 40px rgba(45, 106, 79, 0.05)', border: '1px solid rgba(255, 255, 255, 0.6)' },
  logo: { display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '32px', justifyContent: 'center' },
  logoTitle: { fontSize: '22px', fontWeight: 800, color: '#1B4332', letterSpacing: '-0.5px' },
  logoSub: { fontSize: '12px', color: '#52796F', fontWeight: 500, marginTop: '2px' },
  form: { display: 'flex', flexDirection: 'column', gap: '12px' },
  input: { padding: '13px 16px', borderRadius: '12px', border: '1px solid #CBDCD0', background: '#FFF', fontSize: '14px', outline: 'none', color: '#1B4332', boxSizing: 'border-box' },
  whatsappRow: { display: 'flex', alignItems: 'center', width: '100%' },
  whatsappPrefix: { padding: '13px 14px', background: '#E6EFE9', border: '1px solid #CBDCD0', borderRadius: '12px 0 0 12px', fontSize: '14px', color: '#2D6A4F', fontWeight: 600, boxSizing: 'border-box', borderRight: 'none' },
  whatsappInput: { flex: 1, padding: '13px 16px', borderRadius: '0 12px 12px 0', border: '1px solid #CBDCD0', background: '#FFF', fontSize: '14px', outline: 'none', color: '#1B4332', boxSizing: 'border-box' },
  notice: { background: '#EAF7F0', border: '1px solid #C6EDD4', borderRadius: '12px', padding: '12px 14px', fontSize: '12px', color: '#2D6A4F', lineHeight: '1.5', fontWeight: 500 },
  button: { padding: '14px', borderRadius: '12px', background: '#2D6A4F', color: '#FFF', fontWeight: 600, fontSize: '14px', border: 'none', cursor: 'pointer', marginTop: '6px', boxShadow: '0 4px 12px rgba(45, 106, 79, 0.15)' },
  error: { color: '#E63946', fontSize: '13px', textAlign: 'center', margin: '4px 0 0', fontWeight: 500 },
  link: { display: 'block', textAlign: 'center', marginTop: '24px', color: '#2D6A4F', fontSize: '13px', fontWeight: 600, textDecoration: 'none' },
};